        ...
        rows.append(
            {
                ...
                "no_vig_probability": round(candidate.no_vig_probability, 4),
...
<body>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Implied</th><th>No-vig</th><th>Model</th><th>Confidence</th><th>Edge (pp)</th><th>EV/$</th><th>Kelly</th><th>Stake</th>
          </tr>
        </thead>
        <tbody id="rows"></tbody>
      </table>
  ...
  <script>
    ...
    function renderRows(rows) {{
      ...
      if (!rows.length) {{
        body.innerHTML = `<tr><td colspan="13" class="muted">No opportunities found for current filters.</td></tr>`;
      }} else {{
        body.innerHTML = rows.map((r) => `
          <tr>
            ...
            <td>${{pct(r.no_vig_probability)}}</td>
            <td>${{pct(r.model_probability)}}</td>
            <td>${{n(r.confidence_score,2)}}</td>
