...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--home-advantage-pp", type=float, default=2.0, help="Home-ice boost in win-probability points")
    parser.add_argument("--market-blend", type=float, default=0.2, help="Blend weight toward market no-vig consensus (0-0.85)")
    parser.add_argument("--kelly-scale", type=float, default=0.5, help="Fractional Kelly scale between 0.05 and 1.0")
...
def _print_human(recommendations: list[dict[str, object]]) -> None:
    ...
    for item in recommendations:
        ...
        print(
            ...
            f"No-vig: {c.no_vig_probability:.3f} | Model: {c.model_probability:.3f} | "
            f"Edge {c.edge_probability_points:.2f}pp | EV/$ {c.expected_value_per_dollar:.3f} | "
            f"Conf {c.confidence_score:.2f} | Stake ${item['recommended_stake']} ({item['stake_fraction'] * 100:.2f}% BR)"
        )
...
def main() -> int:
    ...
    config = TrackerConfig(
        ...
        home_advantage_pp=args.home_advantage_pp,
